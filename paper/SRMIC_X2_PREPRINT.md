...
In the current branch, admission is the most reliable source of real-trace gains. Across the public OPT and Qwen runs, the admission-enabled policy consistently produces the highest or tied-highest replay speedup and hit rate.
...
The real-trace results are the key empirical validation tier for this paper. Across the current public-model runs, `x2_admission` outperforms the legacy `srmic` policy in every case and is the strongest or tied-strongest X2 configuration, indicating that remap + admission is presently the safest default policy configuration.

### 10.6 Replication and Workload Sensitivity
Replication provided measurable gains in the `HOTSPOT_FANOUT` workload.
Replication still provides a measurable gain in the `HOTSPOT_FANOUT` workload, reducing synthetic latency proxy by 5.7%. However, in the current real-trace experiments it trails `x2_admission` on the OPT models and on `Qwen2.5-0.5B-Instruct`, while tying it on `Qwen2.5-1.5B-Instruct` under the present short-trace setup. Replication should therefore be positioned as a secondary optimization layer.
...
Code, deterministic synthetic experiments, and public-model trace-replay studies are available in the project repository under the `feature/srmic-x2-controller` branch. The current real-trace results correspond to `facebook/opt-125m`, `facebook/opt-350m`, `facebook/opt-1.3b`, `Qwen/Qwen2.5-0.5B-Instruct`, and `Qwen/Qwen2.5-1.5B-Instruct` replayed through `srmic`, `x2_admission`, and `x2_full`.
