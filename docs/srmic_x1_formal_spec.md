...
SRMIC-X1 is designed to reduce HBM bottlenecks during transformer decode by serving the active per-token working set from distributed on-package SRAM regions connected by a high-bandwidth fabric.
...
SRMIC-X1 is not a general GPU replacement. It is a decode-optimized inference accelerator designed for low-latency memory-bound transformer workloads.
